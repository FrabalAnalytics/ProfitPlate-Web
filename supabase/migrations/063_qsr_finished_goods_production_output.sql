-- Complete the QSR / fast-food operating model introduced by migration 062.
--
-- Sales depletion can now branch row-by-row:
--   * final_product in a sales_outlet => deplete the finished SKU 1-to-1
--   * standard menu item => explode recipe components
--
-- This migration makes the upstream production side capable of creating and
-- crediting those finished goods. Sub-recipes still land in Kitchen/Bar
-- department stock; final menu items can land as manufactured final_product
-- stock in a sales outlet/front counter.

drop function if exists public.create_dashboard_production_run(
  uuid,
  numeric,
  numeric,
  text
...
declare
  ...
  manufactured_item_type public.inventory_item_type;
...
begin
  ...
  manufactured_item_type := case
    when is_final_menu_production then 'final_product'::public.inventory_item_type
  ...
  if is_final_menu_production
     and production_location.location_type <> 'sales_outlet'::public.location_type then
  ...
  where item.organization_id = current_organization_id
    and item.recipe_id = selected_recipe.id
    and item.item_type = manufactured_item_type
