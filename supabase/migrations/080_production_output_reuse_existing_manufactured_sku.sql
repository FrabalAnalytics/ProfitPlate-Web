...
-- location, even when older live data is missing the recipe_id link. This keeps
-- the duplicate master-data guardrail intact while allowing production to post.
...
begin
  ...
  where item.organization_id = current_organization_id
    and item.recipe_id = selected_recipe.id
    and item.item_type::text = manufactured_item_type_value
    and item.cost_type::text = 'manufactured'
      ...
      execute format(
        'update public.inventory_items
            set recipe_id = $1,
                ...
                item_type = $3::%1$s,
                cost_type = $4::%2$s,
                on_hand_uom = coalesce(on_hand_uom, $5, ''unit''),
                current_cost_per_base_uom = coalesce(current_cost_per_base_uom, $6, 0),
