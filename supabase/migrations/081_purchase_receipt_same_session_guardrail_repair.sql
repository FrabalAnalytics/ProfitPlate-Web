-- Receipt confirmation should be blocked for Procurement when they drafted the
-- PO, but live sessions have occasionally stamped created_by with the receiver
-- after role switching. Inventory/Store receivers are the physical control
-- point, so allow those roles to confirm while Procurement remains blocked.
