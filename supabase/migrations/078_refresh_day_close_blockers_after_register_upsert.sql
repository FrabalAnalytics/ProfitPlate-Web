-- Keep operating-day blocker snapshots aligned with live register declarations.
-- Before this, a department could complete its checklist while the manager
-- dashboard still displayed stale blockers saved during an earlier close review.
...
declare
  ...
  selected_operating_date date := coalesce(operating_date_value, current_date);
...
begin
  ...
  update public.operating_days
     set blockers = public.get_dashboard_day_close_blockers(
           target_organization_id,
           selected_operating_date
         )
   where organization_id = target_organization_id
     and operating_date = selected_operating_date
