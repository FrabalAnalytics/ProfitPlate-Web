-- Production output must reuse one canonical manufactured SKU at the output
-- location. Some live workspaces already have duplicate active SUB-/FG- rows;
-- the duplicate guardrail then blocks even repair updates. Consolidate only the
-- selected production output SKU before posting.
...
begin
  ...
  if manufactured_item_id is not null then
    update public.inventory_items canonical
       set on_hand_qty = coalesce(canonical.on_hand_qty, 0)
           + coalesce(duplicate_totals.on_hand_qty, 0)
      from (
        select sum(coalesce(duplicate_item.on_hand_qty, 0)) as on_hand_qty
        from public.inventory_items duplicate_item
        where duplicate_item.organization_id = current_organization_id
          and duplicate_item.location_id is not distinct from production_location_id
          and duplicate_item.id is distinct from manufactured_item_id
          and duplicate_item.is_active = true
          and public.normalize_master_data_key(duplicate_item.sku)
              = public.normalize_master_data_key(manufactured_sku)
      ) duplicate_totals
     where canonical.id = manufactured_item_id
       and canonical.organization_id = current_organization_id
       and coalesce(duplicate_totals.on_hand_qty, 0) <> 0;

    update public.inventory_items duplicate_item
       set is_active = false
     where duplicate_item.organization_id = current_organization_id
       and duplicate_item.location_id is not distinct from production_location_id
       and duplicate_item.id is distinct from manufactured_item_id
